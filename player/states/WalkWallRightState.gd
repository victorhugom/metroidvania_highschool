...
func _enter():
    ...
    agent.wall_walking_direction = agent.WALL_WALK_DIRECTION.RIGHT
    agent.up_direction = Vector2.LEFT

func _update(delta: float):
    if not agent.is_on_floor():
        agent.velocity += Vector2(agent.get_gravity().y, 0) * delta * 5
    
    agent._perform_move_on_wall_right(delta)
    ...
    if int(agent.velocity.y) == 0:
        ...
        agent.emit_signal("state_changed", "walk_wall_right", agent.velocity)